...
class UVV_OT_Relax(unwrap_inplace.UV_OT_uvv_unwrap_inplace):
    ...
    def draw(self, context):
        if self.slim_support and not self.legacy and unwrap_inplace.MULTIPLAYER != 1:
            self.layout.label(text=f'Multiplayer: x{unwrap_inplace.MULTIPLAYER}')
    ...
    def execute(self, context):
        ...
        if not self.slim_support or self.legacy:
            ...
            if not operators or operators[-1].name != 'Relax':
                unwrap_inplace.MULTIPLAYER = 1
                unwrap_inplace.UNIQUE_NUMBER_FOR_MULTIPLY = -1
...
class UVV_OT_Relax_VIEW3D(unwrap_inplace.UV_OT_uvv_unwrap_inplace):
    ...
    def draw(self, context):
        if unwrap_inplace.MULTIPLAYER != 1:
            self.layout.label(text=f'Multiplayer: x{unwrap_inplace.MULTIPLAYER}')
        self.layout.prop(self, 'iterations', slider=True)
        self.layout.prop(self, 'use_correct_aspect')
    ...
    def execute(self, context):
        ...
        # UVV doesn't have univ_settings, use default values
        self.texel = 1.0  # Default texel density
        self.texture_size = 1024.0  # Default texture size
...
classes = [UVV_OT_Relax, UVV_OT_Relax_VIEW3D]
