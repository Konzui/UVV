...
from ..utils.stitch_utils import PaddingHelper
from ..types import (
    BBox,
    UMeshes,
    AdvIslands,
...
from ..properties import univ_settings
...
class Stitch:
    ...
    def stitch(self):
        ...
        for umesh in self.umeshes:
            ...
            if self.between:
                ...
                target_islands = [
                    isl for isl in adv_islands if IslandsBase.island_filter_is_any_edge_selected(isl.faces, umesh)]
    ...
    def set_selected_boundary_tag_with_exclude_face_idx(self, isl, exclude_idx: set):
        """Set boundary tags for selected edges with face index exclusion."""
        uv = isl.umesh.uv
        if self.between:
            if isl.umesh.sync:
                for f in isl:
    ...
    def sort_by_dist_to_mouse_or_sel_edge_length(self, target_islands, umesh):
        if umesh.sync and self.mouse_position:
            target_islands.sort(key=lambda isl: IslandHit.closest_pt_to_selected_edge(isl, self.mouse_position))
        else:
...
class UV_OT_uvv_stitch(Operator, Stitch, PaddingHelper):
    """Stitch selected UV vertices by proximity - UniV implementation"""
    bl_idname = "uv.uvv_stitch"
    bl_label = "Stitch"
    ...
    def execute(self, context):
        """Execute stitch operation - EXACT UniV implementation."""
        ...
        if self.between:
            ...
            if not selected_umeshes and self.mouse_position:
                ...
                hit = CrnEdgeHit(self.mouse_position, self.max_distance)
                for umesh in self.umeshes:
    ...
    def pick_stitch(self, hit: CrnEdgeHit):
        sync = hit.umesh.sync
...
class MESH_OT_uvv_stitch(Operator, Stitch, PaddingHelper, RayCast):
    """Stitch from 3D viewport - UniV implementation"""
    bl_idname = "mesh.uvv_stitch"
    bl_label = "Stitch"
    bl_description = "Stitch selected UV vertices by proximity from 3D viewport"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def stitch_between(self):
        """Handle stitch between from 3D viewport."""
    ...
    def stitch_by_edge(self):
        """
        Handle stitch by edge from 3D viewport.
        Based on UniV's UNIV_OT_Stitch_VIEW3D.stitch_by_edge() (stitch_and_weld.py:1195-1224).

        Returns:
            dict or None: {'FINISHED'} if early return needed, None otherwise
        """
        ...
        if not selected_umeshes and self.mouse_pos_from_3d:
            ...
            if hit := self.ray_cast(utils.get_prefs().max_pick_distance):
    ...
    def execute(self, context):
        """
        Execute stitch operation from 3D viewport.
        Based on UniV's UNIV_OT_Stitch_VIEW3D.execute() (stitch_and_weld.py:1166-1184).
        """
...
def register():
    bpy.utils.register_class(UV_OT_uvv_stitch)
    bpy.utils.register_class(MESH_OT_uvv_stitch)


def unregister():
    bpy.utils.unregister_class(UV_OT_uvv_stitch)
    bpy.utils.unregister_class(MESH_OT_uvv_stitch)