# Copied from Mio3 UV addon - UV island and node management classes
import bpy
import bmesh
    ...
    def find_uv_nodes(self, bm, uv_layer, selected=None):
        ...
        def add_uv_node(loop):
            ...
            if key not in uv_nodes:
                ...
                # Find ALL loops at this UV position in the entire mesh
                # This ensures when we move this UV coordinate, ALL edges connected to it move together
                for face in bm.faces:
                    for face_loop in face.loops:
                        if round_uv(face_loop[uv_layer].uv) == key:
                            uv_nodes[key].loops.append(face_loop)

        if self.mode == "EDGE":  # Edge Mode
            # In edge mode, we need to collect UV nodes differently
            # We iterate through faces and check for edges with select_edge
            faces = selected if selected else bm.faces
            for face in faces:
                if face.select:
                    for loop in face.loops:
                        # Check if this loop's edge has UV select_edge
                        if loop[uv_layer].select_edge:
                            # Add both vertices of the edge
                            add_uv_node(loop)
                            add_uv_node(loop.link_loop_next)
        ...
        if self.mode == "EDGE":
            # Build neighbor relationships based on loop connectivity
            for key, node in uv_nodes.items():
                for loop in node.loops:
                    edge = loop.edge
                    # Check if this edge has select_edge enabled
                    if not any(loop[uv_layer].select_edge for loop in edge.link_loops):
                        continue
                    # Connect to next vertex in the face loop
                    for loop in edge.link_loops:
            ...
            # For FACE and VERT modes, build loop-based connectivity
            for key, node in uv_nodes.items():
                for loop in node.loops:
        ...
        # Group connected nodes using DFS
        visited = set()
        groups = []
        ...
        return groups if groups else [{node} for node in uv_nodes.values()]

    @classmethod
    def from_object(cls, obj, bm=None, uv_layer=None, mode="FACE"):
        manager = cls.__new__(cls)
        manager.objects = [obj]
        manager.mode = mode
        manager.island_manager = None
        manager.groups = []
        ...
        uv_groups = manager.find_uv_nodes(bm, uv_layer)
        manager.add_group(obj, bm, uv_layer, uv_groups)
        return manager

    def update_uvmeshes(self):
        unique_objs = {group.obj for group in self.groups if group.obj is not None}
